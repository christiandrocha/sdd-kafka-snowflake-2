...
- **O "162" não corresponde mais a nada.** Aquele número é de 2026-08-04 e antecede a criação das camadas Silver e Gold.
- **Aplicação retroativa concluída em 2026-08-11.** Os 83 testes da Bronze — o único bloco que nunca tinha passado pelo critério — foram revisados um a um. Nenhuma severidade mudou: os 83 continuam em `error`, agora por análise e não por omissão do default. O raciocínio por classe de coluna está no cabeçalho de `dbt/models/bronze/schema.yml`, e o resumo é que toda coluna testada naquela camada é carga estrutural: chave de MERGE, filtro de delete, ordenação da deduplicação, watermark incremental ou chave de agrupamento na Gold. O candidato mais óbvio a `warn`, o `kafka_created_at`, acabou sendo o watermark incremental dos dez models. Com isso, os 185 testes do projeto estão classificados.
